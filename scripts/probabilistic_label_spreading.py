...
def prob_label_spreading(dataset_name, dataset, data_space, prob_label_column, n_data, k, alpha, n_samples, train_split = None, config = "configs/FGMRES.json"):
    ...
    if alpha != 0: # if alpha is zero we do not need to set up the graph structure nor solve the linear system
        ...
        spreaded_info, trials, ind, time_spread, time_setup, time_solver = sampling_and_spreading_efficient(alpha, S, dataset, prob_label_column, classes, config, indices)
    ...
    result = pd.DataFrame({"dataset": dataset_name, "n_data": n_data, "dim_reduction_technique": dim_reduction_technique, "dimension": data_dim, "prob_labels": prob_label_column,\
         'alpha':alpha,'n_samples': n_samples, "k": k, 'RMSE':rmse,'Accuracy':acc, "KL": kl , "MAE": mae, "TV": tv, "runtime": elapsed_time, "config": config}, index=[0])
...
def solve_probabilistic(Matrix, cfg, ind, length, dataset, prob_label_column, sampling_matrix, classes, positives_new, trials):
    ...
    for i in ind:
        e_i = np.zeros(length)
        e_i[i] = 1  #e_i is the i-th unit vector, solution is the information spread by the i-th node throughout the graph
        ...
        # the solution is scaled such that the maximum entry is 1, i.e. the node that spreads the information gets a value of 1
        m = np.max(solution)
        if m > 0: # always the case if the solution is found, otherwise solution is set to zero anyways so no update necessary
            trials += solution/m

        # sample a class according to the probabilistic label of the i-th point to spread the information of this class
        u = np.random.uniform(size=1) # for sampling the class according to the prob. label randomly
        p_sum = 0
        ...
        # print(i, " : ", j, ",")
        # j is now the sampled class index and we can spread the information for this class and store it in positives_new 
        if m > 0:
            positives_new[:,j] += solution/m
        
        sampling_matrix[i][j] += 1 # store how often class j was sampled for point i for comparison
        # print(i, np.argmax(solution), solution[np.argmax(solution)], solution[i]) # to validate that the maximum entry is the entry of the spreading node itself
    # cleanup
...
def sampling_and_spreading_efficient(alpha, S, dataset, prob_label_column, classes, config, ind):
       
    length = len(dataset)

    # store the cumulative feedback obtained for every data point in the vector trials 
    trials = np.zeros(length)

    # the matrix positives_new stores the cumulative spreaded information for every class and every data point
    # sampling_matrix stores how often a class was sampled for every data point
    positives_new = np.zeros(length*len(classes)).reshape(length,len(classes))
    sampling_matrix = np.zeros(length*len(classes)).reshape(length, len(classes))

    I = scipy.sparse.eye(length)
    M = I - alpha * S

    # now we have to solve the linear system efficiently

    sampling_matrix, positives_new, trials, time_setup, time_spread, time_solver = solve_probabilistic(M,config,ind,length,dataset,prob_label_column,sampling_matrix,classes,positives_new,trials)
